...
trait OrderedPoint extends math.Ordered[java.awt.Point] {

  this :java.awt.Point => override def compare (that :java.awt.Point) :Int =  {
    this.x.compare(that.x) match {
...
object ExercisesOption {
  ...
  def sequence[A] (aos: List[Option[A]]) : Option[List[A]] =
    aos.foldRight(Some(Nil):Option[List[A]]) ((ao, acc) => if (ao == None || acc == None) None else {
      val Some(a) = ao
    ...
    case x::xs => {
      ...
      eval match {
        case Some(z) => {
          ...
          val next = traverse(xs) (f)
          next match {
            case Some(y) => Some(z::y)
            case None => None
          }
...
object Tests extends App {
  ...
  assert (Tree.size (Branch(Branch(Leaf(1), Leaf(2)), Leaf(2))) == 5)
  // Exercise 3
